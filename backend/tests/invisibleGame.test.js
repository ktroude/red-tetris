...
//         expect(InvisibleGame.roomId).toBe('room1'); // Check room ID
//         expect(InvisibleGame.owner).toBe(owner); // Check owner
//         expect(InvisibleGame.opponent).toBe(opponent); // Check opponent
//         expect(InvisibleGame.pieceManager).toBeInstanceOf(PieceManager); // Check piece manager instance
//         expect(InvisibleGame.isRunning).toBe(false); // Check initial game state
//     });

//     test('should add players correctly', () => {
//         expect(InvisibleGame.owner).toBe(owner); // Check if owner is set correctly
//         expect(InvisibleGame.opponent).toBe(opponent); // Check if opponent is set correctly
//     });

//     test('should remove players correctly', () => {
//         InvisibleGame.removeOwner();
//         expect(InvisibleGame.owner).toBeNull(); // Check if owner is removed

//         InvisibleGame.removeOpponent();
//         expect(InvisibleGame.opponent).toBeNull(); // Check if opponent is removed
//     });
...
//         // Start the game loop
//         InvisibleGame.startGameLoop(io, owner);
//         InvisibleGame.startGameLoop(io, opponent);
...
//         expect(InvisibleGame.owner.grid).toEqual(InvisibleGame.freezeLinesGrid(0, owner.grid)); // Check if owner's grid updated
//         expect(InvisibleGame.opponent.grid).toEqual(InvisibleGame.freezeLinesGrid(0, opponent.grid)); // Check if opponent's grid updated
...
//         expect(InvisibleGame.isRunning).toEqual(false); // Check if opponent's grid updated
...
//         // Freeze 2 lines
//         const updatedGrid = InvisibleGame.freezeLinesGrid(2, opponent.grid);
